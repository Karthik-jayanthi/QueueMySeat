...
export function calcFanScore(concert, topArtists) {
  ...
  const exactMatch = topArtists.find(a => a.name.toLowerCase() === concert.artist.toLowerCase());
  if (exactMatch) return Math.min(100, Math.round(exactMatch.popularity * 1.1));
  ...
  if (genreMatch) return Math.round(genreMatch.popularity * 0.75);
  return 0;
}
