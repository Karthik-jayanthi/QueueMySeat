...
export async function fetchRecentlyPlayed(token) {
  // Fetch max 50 tracks, then filter to last 24 hours only
  const data = await spotifyFetch('/me/player/recently-played?limit=50', token);
  const cutoff = Date.now() - 24 * 60 * 60 * 1000; // 24 hours ago in ms
  const recent = (data.items || []).filter(item => {
    if (!item.played_at) return false;
    return new Date(item.played_at).getTime() > cutoff;
  });
  return { items: recent };
}
