...
const ProgBar = ({ pct, color, height }) => (
  <div style={{ background:'#ffffff18', borderRadius:4, height:height||5, overflow:'hidden' }}>
    <div style={{ width:Math.min(100,pct)+'%', background:color||'#7b2ff7', height:'100%', borderRadius:4, transition:'width 0.5s ease' }} />
  </div>
);
const Spinner = () => (
...
function ScoreBreakdown({ scoreData, concert }) {
  ...
  const rows = [
    { label: 'Artist Match',       pts: breakdown.artistMatch,      max: 50, reason: breakdown.artistMatchReason },
    { label: 'Popularity Weight',  pts: breakdown.popularityWeight, max: 20, reason: breakdown.matchedArtist ? breakdown.matchedArtist.popularity + '/100 popularity on Spotify' : 'N/A' },
    { label: 'Listening Recency',  pts: breakdown.listeningRecency, max: 20, reason: breakdown.recencyReason },
    { label: 'Genre Depth',        pts: breakdown.genreDepth,       max: 10, reason: breakdown.genreDepthReason },
  ];
  return (
    <div style={{ background:'#0f0f0f', border:'1px solid #2a2a2a', borderRadius:12, padding:16, marginBottom:16 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
        <div>
          ...
          <div style={{ display:'flex', alignItems:'baseline', gap:8 }}>
            <span style={{ fontSize:36, fontWeight:900, color:ti.color }}>{total}</span>
      ...
      <ProgBar pct={total} color={ti.color} height={8} />
      <div style={{ display:'flex', justifyContent:'space-between', fontSize:10, color:'#555', marginTop:4, marginBottom:12 }}>
        <span>0</span><span style={{ color:ti.color, fontWeight:700 }}>{total}</span><span>100</span>
      </div>
      {rows.map(row => (
      ...
      {tier === 'casual' && (
        <div style={{ background:'#ef444411', border:'1px solid #ef444433', borderRadius:8, padding:10, marginTop:8 }}>
          ...
          <div style={{ fontSize:11, color:'#f87171', marginTop:4 }}>Your fan score ({total}/100) is below the 85 minimum required for early-bird booking. Listen to more {concert.artist} music on Spotify to increase your score.</div>
        </div>
      )}
      {tier === 'growing' && (
        <div style={{ background:'#7b2ff711', border:'1px solid #7b2ff733', borderRadius:8, padding:10, marginTop:8 }}>
          <div style={{ fontSize:11, color:'#a78bfa' }}>You can book but you're {85 - total} points away from Elite Fan status and the loyalty bonus.</div>
        </div>
      )}
    </div>
  );
...
function BookingModal({ concert, topArtists, recentTracks, onClose, onSuccess, loyaltyUsed, setLoyaltyUsed }) {
  const [step, setStep] = useState(1); // 1=score check, 2=details, 3=confirm, 4=processing
  ...
  // Score penalty from +2 friends and extra tickets
        ...
        {step===1 && (
          <>
            ...
            {canBook ? (
              ...
              <>
                <button style={PB(true)} disabled>Early-Bird Booking Blocked</button>
                <div style={{ fontSize:12, color:'#888', textAlign:'center', marginTop:10 }}>
                  Listen to more {concert.artist} on Spotify to increase your score above 85.
                </div>
              </>
            )}
          ...
          <>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
              <div>
                ...
                <h2 style={{ fontSize:20, fontWeight:900, margin:0 }}>{concert.artist}</h2>
              </div>
              <button onClick={()=>setStep(1)} style={{ background:'#1e1e1e', border:'none', color:'#888', borderRadius:8, padding:'5px 10px', cursor:'pointer', fontSize:12 }}>← Back</button>
            </div>

            {/* Mini score */}
            <div style={{ background:'#1a1a1a', borderRadius:10, padding:'10px 14px', marginBottom:16, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                <Badge label={ti.label.toUpperCase()} color={ti.color} />
                <span style={{ fontSize:13, color:'#888' }}>Fan Score:</span>
                <span style={{ fontSize:15, fontWeight:800, color:ti.color }}>{fanScore}/100</span>
              </div>
              <div style={{ textAlign:'right' }}>
                <div style={{ fontSize:11, color:'#888' }}>Queue position</div>
                <div style={{ fontSize:16, fontWeight:900, color:'#a78bfa' }}>#{queuePos}</div>
              </div>
            </div>

            <div style={{ background:'#7b2ff711', border:'1px solid #7b2ff733', borderRadius:10, padding:14, marginBottom:16 }}>
              <div style={{ fontSize:12, color:'#a78bfa', fontWeight:700, marginBottom:4 }}>🎫 Early-Bird Pass</div>
              <div style={{ fontSize:13, color:'#ccc' }}>Your pass lets you queue <strong style={{ color:'#fff' }}>1 + 2 friends</strong> at the same early-bird price. Adding friends will reduce your fan score by 8 pts.</div>
            </div>
            ...
            <label style={{ display:'flex', alignItems:'center', gap:12, border:'1px solid '+(addFriends?'#7b2ff755':'#222'), background:addFriends?'#7b2ff70a':'transparent', borderRadius:10, padding:14, marginBottom:12, cursor:'pointer' }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:11, color:'#888' }}>2 extra seats at {INR(concert.earlyBirdPrice)} each · Fan score −8 pts</div>
              </div>
            </label>
            ...
            {canLoyalty && (
              <div style={{ border:'1px solid #f59e0b44', borderRadius:10, padding:14, marginBottom:12, background:'#f59e0b06' }}>
                <div style={{ fontSize:12, color:'#f59e0b', fontWeight:700, marginBottom:6 }}>⭐ Elite Fan Loyalty Bonus</div>
                <div style={{ fontSize:11, color:'#ccc', marginBottom:10 }}>Score {fanScore}/100 unlocks up to 2 extra tickets. Each costs {INR(concert.earlyBirdPrice)} and reduces your score by 6 pts.</div>
                <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            ...
            {totalPenalty > 0 && (
              <div style={{ background:'#f59e0b0d', border:'1px solid #f59e0b33', borderRadius:8, padding:10, marginBottom:12 }}>
                <div style={{ fontSize:12, color:'#f59e0b', fontWeight:600 }}>Score Impact</div>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:'#ccc', marginTop:4 }}>
                  <span>Current: <strong style={{ color:ti.color }}>{fanScore}</strong></span>
                  <span>After booking: <strong style={{ color: newScore >= 85 ? '#10b981' : newScore >= 70 ? '#f59e0b' : '#ef4444' }}>{newScore}</strong></span>
                </div>
              </div>
            )}
            ...
            <button style={PB(false)} onClick={()=>setStep(3)}>Continue to Payment →</button>
            <button style={SB} onClick={onClose}>Cancel</button>
          </>
        )}
        ...
        {step===3 && (
          <>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
              ...
              <button onClick={()=>setStep(2)} style={{ background:'#1e1e1e', border:'none', color:'#888', borderRadius:8, padding:'5px 11px', cursor:'pointer', fontSize:12 }}>← Back</button>
            </div>
            <div style={{ background:'#1a1a1a', borderRadius:12, padding:16, marginBottom:18 }}>
              {[
                ...
                <div key={k} style={IR}><span style={{ color:'#888' }}>{k}</span><span style={{ fontWeight:500, color:k==='Queue Position'?'#a78bfa':k==='Fan Score'?ti.color:'#f0f0f0' }}>{v}</span></div>
              ))}
            ...
            {totalPenalty > 0 && (
              <div style={{ background:'#f59e0b0d', border:'1px solid #f59e0b33', borderRadius:10, padding:12, marginBottom:14 }}>
                <div style={{ fontSize:12, color:'#f59e0b', fontWeight:600 }}>⚠ Score after booking: {newScore}/100 ({totalPenalty} pts deducted)</div>
              </div>
            )}
            ...
            <button style={PB(false)} onClick={confirmBooking}>🎉 Confirm Booking — {INR(total)}</button>
            <button style={SB} onClick={onClose}>Cancel</button>
          </>
        )}
      </div>
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.9)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ background:'#141414', border:'1px solid #2a2a2a', borderRadius:20, padding:26, maxWidth:400, width:'100%', textAlign:'center' }}>
        ...
        <div style={{ background:'#7b2ff711', border:'1px solid #7b2ff733', borderRadius:14, padding:20, marginBottom:16 }}>
          <div style={{ fontSize:48, fontWeight:900, color:'#a78bfa', lineHeight:1 }}>#{data.queuePos}</div>
          <div style={{ color:'#888', fontSize:12, marginTop:4 }}>Queue position</div>
        ...
        <div style={{ display:'flex', gap:16, justifyContent:'center', marginBottom:16 }}>
          <div><div style={{ fontSize:22, fontWeight:900, color:'#7b2ff7' }}>{data.tickets}</div><div style={{ fontSize:11, color:'#888' }}>Tickets</div></div>
          <div><div style={{ fontSize:22, fontWeight:900, color:'#7b2ff7' }}>{INR(data.total)}</div><div style={{ fontSize:11, color:'#888' }}>Total</div></div>
          <div><div style={{ fontSize:22, fontWeight:900, color:ti.color }}>{data.newScore}</div><div style={{ fontSize:11, color:'#888' }}>New Score</div></div>
        </div>
                ...
                {topArtists.slice(0,5).map((artist,i)=>(
                  <div key={artist.id} style={{ background:'#141414', border:'1px solid #1e1e1e', borderRadius:12, padding:13, display:'flex', alignItems:'center', gap:13 }}>
                    ...
                    <div style={{ textAlign:'right' }}>
                      ...
                      {artist.popularity>=85&&<Badge label="ELITE" color="#f59e0b"/>}
                      {artist.popularity>=70&&artist.popularity<85&&<Badge label="VERIFIED" color="#10b981"/>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              return (
                <div key={c.id} style={{ background:'#141414', border:'1px solid '+c.accent+'22', borderRadius:12, padding:14 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:8 }}>
                    ...
                    <div style={{ textAlign:'right' }}>
                      <div style={{ fontSize:18, fontWeight:900, color:ti2.color }}>{sd.total}/100</div>
                      <Badge label={ti2.label.toUpperCase()} color={ti2.color} />
                    </div>
                  </div>
                  <ProgBar pct={sd.total} color={ti2.color} height={4} />
