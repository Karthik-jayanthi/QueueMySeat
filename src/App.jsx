...
function ArtistProfileModal({ artist, recentTracks, token, onClose }) {
  ...
  useEffect(() => {
    ...
    fetchArtistTopTracks(artist.id, token)
      .then(data => { setTopTracks(data.tracks || []); setLoadingTracks(false); })
      .catch(() => setLoadingTracks(false));
  }, [artist, token]);

  if (!artist) return null;

  // Count how many times this artist appears in recently played
  const playCount = recentTracks.filter(item =>
    item.track?.artists?.some(a => a.id === artist.id)
  ).length;

  // Minutes listened estimate (avg song ~3.5 min)
  const estimatedMinutes = Math.round(playCount * 3.5);

  // Most repeated song from recent tracks for this artist
  ...
  // Most & least popular from Spotify top tracks
  const sortedByPop = [...topTracks].sort((a,b) => b.popularity - a.popularity);
      ...
      <div style={{ background:'#141414', border:'1px solid #2a2a2a', borderRadius:20, padding:24, maxWidth:520, width:'100%', maxHeight:'90vh', overflowY:'auto' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}>
          <div style={{ display:'flex', gap:14, alignItems:'center' }}>
            <Avatar src={artist.images?.[0]?.url} initials={artist.name[0]} color="#7b2ff7" size={64} />
            <div>
              <div style={{ fontSize:22, fontWeight:900 }}>{artist.name}</div>
              <div style={{ fontSize:12, color:'#888', marginTop:3 }}>{artist.genres?.slice(0,3).join(', ')}</div>
              <div style={{ fontSize:12, color:'#1DB954', marginTop:4 }}>{(artist.followers?.total||0).toLocaleString()} followers</div>
            </div>
          </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:20 }}>
          {[
            ...
            ['Times Played', playCount > 0 ? playCount + 'x' : 'N/A'],
            ['Est. Minutes', estimatedMinutes > 0 ? estimatedMinutes + ' min' : 'N/A'],
          ].map(([l,v]) => (
            <div key={l} style={{ background:'#1a1a1a', borderRadius:10, padding:'12px 14px', textAlign:'center' }}>
        ...
        {/* Most repeated from your history */}
        {mostRepeated && (
          <div style={{ background:'#7b2ff711', border:'1px solid #7b2ff733', borderRadius:10, padding:12, marginBottom:16 }}>
            <div style={{ fontSize:11, color:'#a78bfa', fontWeight:700, marginBottom:4 }}>🔁 Most Repeated in Your History</div>
            <div style={{ fontWeight:600 }}>{mostRepeated[0]}</div>
            <div style={{ fontSize:11, color:'#888' }}>Played {mostRepeated[1]}x in your recent history</div>
          </div>
        )}

        {/* Top tracks from Spotify */}
        {loadingTracks ? <Spinner /> : topTracks.length > 0 && (
          <>
            {mostPopular && (
              <div style={{ background:'#10b98111', border:'1px solid #10b98133', borderRadius:10, padding:12, marginBottom:10 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <img src={mostPopular.album?.images?.[2]?.url} alt="" style={{ width:40, height:40, borderRadius:6, objectFit:'cover' }} />
                  <div style={{ flex:1 }}>
            ...
            {leastPopular && leastPopular.id !== mostPopular?.id && (
              <div style={{ background:'#ef444411', border:'1px solid #ef444433', borderRadius:10, padding:12, marginBottom:16 }}>
                <div style={{ fontSize:11, color:'#ef4444', fontWeight:700, marginBottom:6 }}>💎 Least Known Track You've Heard</div>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <img src={leastPopular.album?.images?.[2]?.url} alt="" style={{ width:40, height:40, borderRadius:6, objectFit:'cover' }} />
                  <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:13, fontWeight:700, color:'#888', marginBottom:10 }}>Top Tracks on Spotify</div>
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              {topTracks.slice(0,5).map((track, i) => (
                <div key={track.id} style={{ display:'flex', alignItems:'center', gap:10, background:'#1a1a1a', borderRadius:8, padding:'8px 12px' }}>
                  ...
                  <img src={track.album?.images?.[2]?.url} alt="" style={{ width:34, height:34, borderRadius:5, objectFit:'cover' }} />
                  <div style={{ flex:1 }}>
