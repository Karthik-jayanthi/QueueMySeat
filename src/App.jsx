...
const T = {
  ...
  text: '#ffffff', textMuted: '#555', textSub: '#333',
  green: '#1DB954', white: '#ffffff',
};

const Avatar = ({ src, initials, color, size }) => {
  const [err, setErr] = useState(false);
  const sz = size || 44;
  const base = { width: sz, height: sz, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: Math.round(sz * 0.33), color: '#000', background: color || '#fff', letterSpacing: 0.5, overflow: 'hidden' };
...
const Chip = ({ label, color }) => (
  <span style={{ background: 'transparent', color: color || '#555', border: `1px solid ${color || '#333'}`, fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 3, letterSpacing: 1, textTransform: 'uppercase' }}>{label}</span>
);
      ...
      <div>
        {rows.map((row, i) => (
          <div key={row.label} style={{ padding: '12px 20px', borderBottom: i < rows.length - 1 ? `1px solid ${T.border}` : 'none', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ width: 80 }}><ProgBar pct={(row.pts / row.max) * 100} color={row.pts > 0 ? ti.color : '#222'} height={2} /></div>
...
Do not use quotes or asterisks. Start with "You'll love this because" or a similar hook.`;
      ...
      if (!GEMINI_API_KEY) throw new Error('Gemini API key not configured. Add VITE_GEMINI_API_KEY to Vercel env variables.');
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      <button onClick={handleClick} style={{ width: '100%', background: open ? '#0a0a0a' : 'transparent', border: `1px solid ${open ? '#2a2a2a' : '#1a1a1a'}`, borderRadius: 7, padding: '7px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', transition: 'all 0.2s' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
            <path d="M12 2L14.09 9.26L21 12L14.09 14.74L12 22L9.91 14.74L3 12L9.91 9.26L12 2Z" fill="#4f8ef7" />
          </svg>
          <span style={{ fontSize: 11, fontWeight: 700, color: '#4f8ef7', letterSpacing: 0.3 }}>Why this show?</span>
        </div>
      ...
      {open && (
        <div style={{ borderLeft: '2px solid #4f8ef722', marginTop: 6, padding: '8px 12px', background: '#050508', borderRadius: '0 6px 6px 0' }}>
          {loading && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 12, height: 12, border: '1.5px solid #222', borderTop: '1.5px solid #4f8ef7', borderRadius: '50%', animation: 'spin 0.7s linear infinite', flexShrink: 0 }} />
              <span style={{ fontSize: 11, color: '#555' }}>Personalizing for your taste…</span>
            </div>
          )}
          {error && <span style={{ fontSize: 11, color: '#ef4444' }}>{error}</span>}
          {reason && <p style={{ fontSize: 12, color: '#aaa', lineHeight: 1.6, margin: 0 }}>{reason}</p>}
        </div>
      )}
    </div>
  );
...
function ConcertCard({ concert, onBook, topArtists, recentTracks, loggedIn }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ background: T.bg, border: `1px solid ${hovered ? '#333' : T.border}`, borderRadius: 12, overflow: 'hidden', transition: 'border-color 0.2s, transform 0.2s', transform: hovered ? 'translateY(-2px)' : 'none' }}>
      <div style={{ height: 2, background: concert.accent }} />
      <div style={{ padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <div style={{ width: 40, height: 40, borderRadius: 8, background: concert.accent + '18', border: `1px solid ${concert.accent}33`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: concert.accent }}>{concert.initials}</div>
            <div>
          ...
          {loggedIn && fanScore > 0 && <Chip label={ti.label} color={ti.color} />}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => onBook(concert)} disabled={isSoldOut}
            style={{ background: isSoldOut ? T.surface : T.white, color: isSoldOut ? T.textMuted : T.bg, border: `1px solid ${isSoldOut ? T.border : T.white}`, borderRadius: 7, padding: '9px 16px', fontWeight: 800, fontSize: 12, cursor: isSoldOut ? 'not-allowed' : 'pointer' }}>
...
// ─── Booking Modal ────────────────────────────────────────
function BookingModal({ concert, topArtists, recentTracks, spotifyId, onClose, onSuccess, loyaltyUsed, setLoyaltyUsed }) {
  ...
  const confirmBooking = async () => {
    ...
    setSaveError(null);
    try {
      await saveBooking(spotifyId, { concert, queuePos, tickets, total, addFriends, extraTickets, fanScore, newScore });
    } catch (err) {
      setSaveError('Could not save booking. Check your connection and try again.');
      setProcessing(false);
      return;
    }
  ...
  const OL = { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 };
  const BOX = { background: T.bg, border: `1px solid ${T.border}`, borderRadius: 16, padding: 28, maxWidth: 480, width: '100%', maxHeight: '92vh', overflowY: 'auto' };
  const PrimaryBtn = ({ onClick, disabled, children }) => (
    <button onClick={onClick} disabled={disabled} style={{ background: disabled ? T.surface : T.white, color: disabled ? T.textMuted : T.bg, border: `1px solid ${disabled ? T.border : T.white}`, borderRadius: 8, padding: '13px 24px', fontWeight: 800, fontSize: 14, cursor: disabled ? 'not-allowed' : 'pointer', width: '100%', marginTop: 8 }}>{children}</button>
  );
  const SecondaryBtn = ({ onClick, children }) => (
    <button onClick={onClick} style={{ background: 'transparent', color: T.textMuted, border: `1px solid ${T.border}`, borderRadius: 8, padding: '11px 20px', fontWeight: 500, fontSize: 13, cursor: 'pointer', width: '100%', marginTop: 8 }}>{children}</button>
  );
      ...
      <div style={BOX}>
        {processing && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <Spinner />
            <div style={{ fontSize: 15, fontWeight: 700, marginTop: 12 }}>Saving your booking...</div>
            <div style={{ color: T.textMuted, fontSize: 12, marginTop: 6 }}>Securing your early-bird position</div>
          </div>
        )}
            ...
            <div style={{ border: `1px solid ${T.border}`, borderRadius: 10, padding: 16, marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 900 }}>#{queuePos}</div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 20, marginTop: 14, paddingTop: 14, borderTop: `1px solid ${T.border}` }}>
                <div><div style={{ fontSize: 9, color: T.textMuted, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>Fan score</div><div style={{ fontSize: 15, fontWeight: 800, color: ti.color }}>{fanScore}/100</div></div>
                <div><div style={{ fontSize: 9, color: T.textMuted, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>Status</div><div style={{ fontSize: 15, fontWeight: 800, color: ti.color }}>{ti.label}</div></div>
                <div><div style={{ fontSize: 9, color: T.textMuted, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>Seats left</div><div style={{ fontSize: 15, fontWeight: 800 }}>{rem}</div></div>
              </div>
            </div>
            <div onClick={() => setAddFriends(!addFriends)}
              style={{ border: `1px solid ${addFriends ? '#333' : T.border}`, borderRadius: 10, padding: 16, marginBottom: 12, cursor: 'pointer', background: addFriends ? '#111' : 'transparent' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 20, height: 20, borderRadius: 4, border: `1px solid ${addFriends ? T.white : T.border}`, background: addFriends ? T.white : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            {canLoyalty && (
              <div style={{ border: `1px solid #f59e0b33`, borderRadius: 10, padding: 16, marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  {[0, 1, 2].map(n => (
                    <button key={n} onClick={() => setExtraTickets(n)} style={{ background: extraTickets === n ? T.white : T.surface, color: extraTickets === n ? T.bg : T.textMuted, border: `1px solid ${extraTickets === n ? T.white : T.border}`, borderRadius: 6, padding: '7px 14px', cursor: 'pointer', fontWeight: 800, fontSize: 14 }}>{n}</button>
                  ))}
                </div>
              </div>
            )}
        ...
        {!processing && step === 3 && (
          <>
            ...
            <div style={{ border: `1px solid ${T.border}`, borderRadius: 10, padding: '14px 16px', marginBottom: 16 }}>
              {[['Artist', concert.artist], ['Date', fmtDate(concert.date) + ' · ' + concert.time], ['Venue', concert.venue + ', ' + concert.city], ['Tickets', tickets + (addFriends ? ' (you + 2 friends)' : '') + (extraTickets > 0 ? ' + ' + extraTickets + ' loyalty' : '')], ['Queue position', '#' + queuePos], ['Fan score', fanScore + '/100 — ' + ti.label]].map(([k, v]) => (
                <Row key={k} label={k} value={v} valueColor={k === 'Queue position' ? '#888' : k === 'Fan score' ? ti.color : T.text} />
              ))}
              <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 12, marginTop: 4 }}>
                <span style={{ fontSize: 15, fontWeight: 700 }}>Total</span>
                <span style={{ fontSize: 22, fontWeight: 900 }}>{INR(total)}</span>
            ...
            {saveError && (
              <div style={{ border: `1px solid #ef444433`, borderRadius: 8, padding: '10px 14px', marginBottom: 12, background: '#ef44440a' }}>
            ...
            <div style={{ border: `1px solid #1DB95422`, borderRadius: 8, padding: '10px 14px', marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: '#1DB954' }}>Secure checkout · Early-bird price locked · Saved to your account</div>
            </div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 16, padding: 32, maxWidth: 400, width: '100%', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 72, fontWeight: 900, lineHeight: 1, marginBottom: 4 }}>#{data.queuePos}</div>
        ...
        <div style={{ display: 'flex', border: `1px solid ${T.border}`, borderRadius: 10, overflow: 'hidden', marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 11, color: '#1DB954', marginBottom: 20 }}>✓ Booking saved to your account permanently</div>
        <button onClick={onViewTickets} style={{ background: T.white, color: T.bg, border: 'none', borderRadius: 8, padding: '13px 24px', fontWeight: 800, fontSize: 14, cursor: 'pointer', width: '100%', marginBottom: 8 }}>View my tickets</button>
...
setTopArtists(artists);
  ...
  const handleLogout = () => {
    ...
    setTopArtists([]); setRecentTracks([]);
    setBookedList([]); setPage('home');
  };
  ...
  const NavBtn = ({ label, p }) => (
    <button onClick={() => setPage(p)} style={{ background: 'transparent', color: page === p ? T.text : T.textMuted, border: `1px solid ${page === p ? T.border : 'transparent'}`, borderRadius: 6, padding: '5px 12px', cursor: 'pointer', fontSize: 12, fontWeight: page === p ? 600 : 400 }}>{label}</button>
  );
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, color: T.text }}>
      <nav style={{ position: 'sticky', top: 0, zIndex: 200, height: 56, background: 'rgba(0,0,0,0.97)', borderBottom: `1px solid ${T.border}`, padding: '0 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        ...
        <>
          <div style={{ padding: '72px 28px 56px', borderBottom: `1px solid ${T.border}` }}>
            <div style={{ maxWidth: 680, margin: '0 auto' }}>
              ...
              <h1 style={{ fontSize: 'clamp(36px,6vw,72px)', fontWeight: 900, letterSpacing: -3, lineHeight: 1, marginBottom: 20 }}>Real fans.<br />Real access.</h1>
              <p style={{ fontSize: 16, color: T.textMuted, lineHeight: 1.7, maxWidth: 480, marginBottom: 36 }}>Connect your Spotify account. Prove your fandom through your listening history. Get priority early-bird access before bots and resellers.</p>
              {!token ? (
              ...
              <div style={{ display: 'flex', gap: 40, marginTop: 52, paddingTop: 28, borderTop: `1px solid ${T.border}` }}>
                {[['15+', 'upcoming shows'], ['8', 'cities'], ['85+', 'score to book']].map(([n, l]) => (
                  <div key={l}><div style={{ fontSize: 24, fontWeight: 900 }}>{n}</div><div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>{l}</div></div>
                ))}
              </div>
            </div>
          </div>
          <div style={{ padding: '14px 28px', borderBottom: `1px solid ${T.border}`, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <input placeholder="Search artist or city..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 1, borderBottom: `1px solid ${T.border}` }}>
                  ...
                  recentTracks.map((item, i) => {
                    ...
                    return (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 18px', borderBottom: i < recentTracks.length - 1 ? `1px solid ${T.border}` : 'none' }}>
                        ...
                        <div style={{ fontSize: 10, textAlign: 'right', flexShrink: 0, marginLeft: 12 }}>
                          <div style={{ fontWeight: 600, color: T.text }}>{timeLabel}</div>
                          <div style={{ color: T.textMuted, marginTop: 2 }}>{playedAt ? playedAt.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) : ''}</div>
                        </div>
                      </div>
                    );
                  })
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {bookedList.map((entry, i) => (
                <div key={i} style={{ border: `1px solid ${T.border}`, borderRadius: 12, overflow: 'hidden' }}>
                  <div style={{ height: 2, background: entry.concert.accent || '#fff' }} />
      ...
      {successModal && (
        <SuccessModal
          ...
          onViewTickets={() => { setSuccessModal(null); setPage('tickets'); }}
        />
      )}
    </div>
  );
